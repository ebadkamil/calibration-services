...
def gain_corrected_roi_intensity(module_number, proposal, run, dark_run, *,
                                 pulse_ids=None):
    """ Evaluate gain corrected LPD intensity for each tiles in a module 
    ...
    run: str, int
        A run number such as 243, '243' or 'r0243'.
    dark_run: dict with keys "high", "medium" and "low"
        dark_run["high"] of shape (n_pulses, slow_scan, fast_scan)
    pulse_ids: str
        For eg. ":" to select all pulses in a train
                "start:stop:step" to select indices with certain step size
                "1,2,3" comma separated pulse index to select specific pulses
    ...
    out: xarray
        The first axis of the returned data will be labelled with the trainId
        Shape of numpy array: (n_trains, 16 tiles, mem_celss)
    """
    ...
    for tid, data in run.trains():
        ...
        tiles_slices = split_tiles(image)
        intensities.append(np.mean(tiles_slices, axis=(-1, -2)))
        train_ids.append(tid)
